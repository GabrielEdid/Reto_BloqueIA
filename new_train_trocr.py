...
class TrOCRDataset(Dataset):
    """
    Dataset para fine-tuning de TrOCR en CORD-v2 usando gt_parse.

    En lugar de buscar 'valid_line', construimos el texto objetivo a partir de:
    - store_name, branch, addr, date
    - menu: lista de items con cantidad, nombre y price
    - subtotal, service, tax, total (cuando existan)
    """
    ...
    def extract_text_from_ground_truth(self, ground_truth_str: str) -> str:
        """
        Construye una cadena de texto a partir de gt_parse.
        Espera algo tipo:
        {
          "gt_parse": {
            "store_name": "...",
        ...
        store_name = parse.get("store_name")
        if store_name:
            lines.append(str(store_name).strip())
        
        branch = parse.get("branch")
        if branch:
            lines.append(str(branch).strip())
        
        addr = parse.get("addr")
        if addr:
            lines.append(str(addr).strip())
        
        date = parse.get("date")
        if date:
            lines.append(f"DATE: {str(date).strip()}")
        ...
        if isinstance(menu, list):
            for item in menu:
                ...
                parts = [p for p in [cnt, nm, price] if p]
                if parts:
                    lines.append(" ".join(parts))
        
        subtotal = parse.get("subtotal")
        if subtotal:
            lines.append(f"SUBTOTAL {str(subtotal).strip()}")
        
        service = parse.get("service")
        if service:
            lines.append(f"SERVICE {str(service).strip()}")
        
        tax = parse.get("tax")
        if tax:
            lines.append(f"TAX {str(tax).strip()}")
        
        total = parse.get("total")
        if total:
            lines.append(f"TOTAL {str(total).strip()}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train TrOCR model on CORD-v2 dataset (RTX 4070 Ti optimized)"
    )
    parser.add_argument(
        "--strategy",
        type=str,
